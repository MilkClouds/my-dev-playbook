...
uv self update && uv tool upgrade --all \
  && npm install -g pnpm@latest && pnpm update -g --ignore-scripts \
...
- **Node.js** *(minimal)*: Install [nvm](https://github.com/nvm-sh/nvm): owns the `node` runtime and brings `npm` (still used for `npx`, e.g. Claude Code's MCP servers, and to install pnpm). Global Node CLIs install via [pnpm](#tool-management), not `npm -g`.
