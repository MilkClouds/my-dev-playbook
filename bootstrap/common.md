...
- **System CLIs** (non-pypi — install via `pixi global install <name>`):
  ...
  - **full**: [`tmux`](https://github.com/tmux/tmux) (terminal multiplexer for SSH workflows; doubles as multi-agent workspace), [`ncdu`](https://dev.yorhel.nl/ncdu) (interactive disk usage analyzer).
  - **as-needed**: [`cmake`](https://cmake.org/) (C/C++ build system), [`hyperfine`](https://github.com/sharkdp/hyperfine) (CLI benchmarking), [`ffmpeg`](https://ffmpeg.org/) (audio/video processing), [`typst`](https://typst.app/) (modern typesetting system, LaTeX alternative).
