...
- Default: Fable judges and Opus executes, so specs, design, review, and diagnosis go to Fable, while settled specs and wide mechanical changes go to Opus.
- Escalation: when you stall twice on the same problem, prefer Codex GPT-6 Astra, since different family reliably breaks the anchored hypothesis; if the Codex quota is gone, fall back to the other Claude model and say the cross-family check was skipped.
